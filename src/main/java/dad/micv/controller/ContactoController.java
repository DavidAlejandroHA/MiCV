...
public class ContactoController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		// bindings
		telefonosTable.itemsProperty().bindBidirectional(contacto.telefonosProperty());
		emailTable.itemsProperty().bindBidirectional(contacto.emailsProperty());
		webTable.itemsProperty().bindBidirectional(contacto.websProperty());
	...
	void onAnadirTelefono(ActionEvent event) {
		...
		//dialogo.getDialogPane().getTypeSelector().get
		...
		Optional<Telefono> valores = dialogo.showAndWait();
			if (valores.isPresent()) // El Optional permite comprobar si todos los valores están presentes
				contacto.telefonosProperty().add(new Telefono(dialogo.getNumero(), dialogo.getTipo()));

	}

	@FXML
	void onEliminarTelefono(ActionEvent event) {
		contacto.telefonosProperty().remove(telefonosTable.getSelectionModel().selectedIndexProperty().get());
	}
	...
	void onAnadirCorreo(ActionEvent event) {
		...
		if (valor.isPresent()) // El Optional permite comprobar si todos los valores están presentes
			contacto.emailsProperty().add(new Email(dialogo.getDireccion()));
	}

	@FXML
	void onEliminarCorreo(ActionEvent event) {
		contacto.emailsProperty().remove(emailTable.getSelectionModel().selectedIndexProperty().get());
	}
	...
	void onAnadirWeb(ActionEvent event) {
		...
		if (valor.isPresent()) // El Optional permite comprobar si todos los valores están presentes
			contacto.websProperty().add(new Web(dialogo.getUrl()));
	}

	@FXML
	void onEliminarWeb(ActionEvent event) {
		contacto.websProperty().remove(webTable.getSelectionModel().selectedIndexProperty().get());
	}
